/**
 * Cleanup + Re-import Script for Savvey Savers Members
 * 
 * This script:
 * 1. Deletes all blank/broken user records (those missing name or email)
 * 2. Re-creates all 65 members with correct data fields
 * 
 * Run with: npx tsx scripts/fix_import_users.ts
 */
...
async function runFixImport() {
  ...
  console.log(`Total users found in Firestore: ${snapshot.size}`);
  ...
  snapshot.forEach((doc) => {
    ...
    // Delete records where name or email is blank (broken imports from wrong API call)
    if (!hasName || !hasEmail) {
      console.log(`  🗑  Deleting blank record: ${doc.id} (name="${data.name}", email="${data.email}")`);
  ...
  await batch.commit();
  console.log(`\n✅ Deleted ${deletedCount} blank/broken records. Kept ${skippedCount} valid records.\n`);

  // Now re-import all 65 members correctly
  console.log(`Importing ${members.length} members with full data...`);
  let created = 0;
  let updated = 0;
  ...
  for (const m of members) {
    ...
    // Check if already exists (by invitationId)
    const existingSnap = await usersRef.where('invitationId', '==', m.invitationId).limit(1).get();
    ...
    if (!existingSnap.empty) {
      // Update existing doc
      await existingSnap.docs[0].ref.update(userData);
      console.log(`  ✏️  Updated: ${m.invitationId} - ${m.name}`);
      updated++;
    } else {
      // Check by email
      const byEmailSnap = await usersRef.where('email', '==', email).limit(1).get();
      if (!byEmailSnap.empty) {
        await byEmailSnap.docs[0].ref.update(userData);
        console.log(`  ✏️  Updated by email: ${m.invitationId} - ${m.name}`);
        updated++;
      } else {
        // Create new
        const docId = `mig_${m.invitationId.replace('-', '_')}`;
        await usersRef.doc(docId).set({ id: docId, ...userData });
...
runFixImport().catch(console.error);
